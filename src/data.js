export const arr = [
    {
      img:'data:image/jpeg;base64,',
      name:'Blessing Krofegha',
      skills : ['#Software Engineer','#Writter','#Public Speaker'],
      quote:'When i’m not coding i switch to netflix with biscuits and cold tea as my companion.' 
    },
    {
        img:'https://images.pexels.com/photos/3031396/pexels-photo-3031396.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500',
        name:'Collis',
        skills : ['#Software Engineer','#Writter','#Public Speaker'],
        quote:'My Neck, My Back, My NETFLIX and My Snacks' 
      },
      {
        img:'https://images.pexels.com/photos/1987301/pexels-photo-1987301.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500',
        name:'Tomaz Barcellos',
        skills : ['#Software Engineer','#Writter','#Public Speaker'],
        quote:'net.flix.ing' 
      },
      {
        img:'https://images.pexels.com/photos/415829/pexels-photo-415829.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500',
        name:'suno samasaki',
        skills : ['#Software Engineer','#Writter','#Public Speaker'] ,
        quote:'All i do is sit and watch netflix '
      },{
        img:'https://images.pexels.com/photos/614810/pexels-photo-614810.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500',
        name:'Simon Robben',
        skills : ['#Software Engineer','#Writter','#Public Speaker'] ,
        quote:'Netflix is so better than going out and pretending to like people'
      },
      {
        img:'https://images.pexels.com/photos/2379005/pexels-photo-2379005.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500',
        name:'Italo Melo',
        skills : ['#Software Engineer','#Writter','#Public Speaker'] ,
        quote:"Everyone's screwed up in one and mine is Netflix" 
      },
      {
          img:'https://images.pexels.com/photos/2100063/pexels-photo-2100063.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500',
          name:'Ingrid Santana',
          skills : ['#Software Engineer','#Writter','#Public Speaker'],
          quote:'Netflix POPcorn and a cozy blanket'
        },
        {
          img:'https://images.pexels.com/photos/2128807/pexels-photo-2128807.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500',
          name:'David Garrison',
          skills : ['#Software Engineer','#Writter','#Public Speaker'] ,
          quote:'Netflix gives you 15seconds to decide if you are doing anything with your life today'
        },
        {
          img:'https://images.pexels.com/photos/2167673/pexels-photo-2167673.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500',
          name:'Wilson Vitorino',
          skills : ['#Software Engineer','#Writter','#Public Speaker'] ,
          quote:'I NEED TO STOP, I whisphered to myself, as i clicked the Next EPISODE'
        },{
          img:'https://images.pexels.com/photos/167669/pexels-photo-167669.jpeg?auto=compress&cs=tinysrgb&dpr=2&w=500',
          name:'Travis Seera',
          skills : ['#Software Engineer','#Writter','#Public Speaker'] ,
          quote:'Just One More EPISODE'
        }
  ];